...
def get_cifar_anomaly_dataset(trn_img, trn_lbl, tst_img, tst_lbl, abn_cls_idx=0, manualseed=-1):
    ...
    # 무조건 cat[3]은 정상(학습,테스트용), dog[5]는 비정상(테스트용)
    nrm_trn_idx = np.where(trn_lbl == 3)[0]
    abn_trn_idx = np.where(trn_lbl == 5)[0]
    nrm_trn_img = trn_img[nrm_trn_idx]    # Normal training images
    abn_trn_img = trn_img[abn_trn_idx]    # Abnormal training images
    nrm_trn_lbl = trn_lbl[nrm_trn_idx]    # Normal training labels
    abn_trn_lbl = trn_lbl[abn_trn_idx]    # Abnormal training labels.

    nrm_tst_idx = np.where(tst_lbl == 3)[0]
    abn_tst_idx = np.where(tst_lbl == 5)[0]
    nrm_tst_img = tst_img[nrm_tst_idx]    # Normal training images
    abn_tst_img = tst_img[abn_tst_idx]    # Abnormal training images.
    nrm_tst_lbl = tst_lbl[nrm_tst_idx]    # Normal training labels
    abn_tst_lbl = tst_lbl[abn_tst_idx]    # Abnormal training labels.
...
def get_mnist_anomaly_dataset(trn_img, trn_lbl, tst_img, tst_lbl, abn_cls_idx=0, manualseed=-1):
    ...
    # 무조건 1은 정상(학습,테스트용), 0은 비정상(테스트용)
    nrm_trn_idx = torch.from_numpy(np.where(trn_lbl.numpy() == 1)[0])
    abn_trn_idx = torch.from_numpy(np.where(trn_lbl.numpy() == 0)[0])
    nrm_tst_idx = torch.from_numpy(np.where(tst_lbl.numpy() == 1)[0])
    abn_tst_idx = torch.from_numpy(np.where(tst_lbl.numpy() == 0)[0])
