    ...
    def test(self):
        ...
        with torch.no_grad():
            ...
            for i, data in enumerate(self.dataloader['test'], 0):
                ...
                # 테스트로 나온 것을 저장 및 보기 
                reals, fakes, fixed = self.get_current_images()
                self.visualizer.save_current_images_test(reals, fakes, i)
                if self.opt.display:
                    self.visualizer.display_current_images(reals, fakes, fixed)
            ...
            # for i in range(len(self.an_scores)):
            #     print(i,') score:',self.an_scores[i],'gt(normal:0,abnormal:1):',self.gt_labels[i])
            # auc, eer = roc(self.gt_labels, self.an_scores)

            # 산점도 그리기
            plt.figure()
            plt.scatter(range(len( self.an_scores)),  self.an_scores.cpu().numpy(), c=['skyblue' if x == 1 else 'pink' for x in self.gt_labels.cpu().numpy()])
            plt.savefig(os.path.join('./', "scatter.png"))
            plt.close()
            print('산점도 저장 완료!')
