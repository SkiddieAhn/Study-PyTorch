...
class Decoder(nn.Module):
    def __init__(
            self, img_size=[256,256], hidden_sizes=[384,192,96,48], out_channels=3, depths=[2,2,2,2], num_heads=4,
            kernel_size=(2,2), dropout=0.1, attn_drop=0.1, proj_drop=0.1, downsample='conv') -> None:
        super().__init__()

        '''
        If you config kernel_size(2,2), you should config below code too!
        '''
        input_sizes = [(img_size[0]//32) * (img_size[1]//32), (img_size[0]//16) * (img_size[1]//16), (img_size[0]//8) * (img_size[1]//8), (img_size[0]//4) * (img_size[1]//4)] # 16*16, 32*32, 64*64
        ...
        for i in range(3):
            ...
            attn_layers = []
            for _ in range(depths[i]):
                attn_layers.append(SpatialAttn2DBlock(conv_hidden=hidden_sizes[i+1], input_size=input_sizes[i+1], dim=hidden_sizes[i+1], qkv_bias=False, num_heads = num_heads,
                                                    attn_drop=attn_drop,proj_drop=proj_drop,is_pos_embed=True))
            self.attn_stages.append(nn.Sequential(*attn_layers))
        ...
        3. Head
        '''
        self.out = Head(in_channels=hidden_sizes[-1]//4, out_channels=out_channels)
    ...
    def forward(self, fusions, x0):
        ...
        x = self.upsample_layers[-1](x) # 256 x 256 x 12
        x += x0 
        x = self.out(x) # 256 x 256 x 3
...
class PM1(nn.Module):
    def __init__(
        ...
        self.decoder = Decoder(img_size=[img_size[1], img_size[2]], hidden_sizes=r_hidden_sizes, out_channels=in_channels, depths=depths,num_heads=num_heads,
                               kernel_size=[kernel_size[1], kernel_size[2]], dropout=dropout, attn_drop=attn_drop, proj_drop=proj_drop, downsample=downsample)
...
if __name__=="__main__":
    ...
    model = PM1(downsample='pm').to(device)
