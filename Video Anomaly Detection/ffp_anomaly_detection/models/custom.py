...
class Generator4(nn.Module):
    def __init__(self, in_ch, out_ch):
        ...
        bn_hw_size = 64//(2**(self.n_layers-1)) # input_resolution: 64x64 -> after encoder -> 8x8
...
if __name__=="__main__":
    in_channels = 3
    out_channels = 3
    rand_input = torch.rand([1, 3, 4, 256, 256])
    model = Generator4(in_channels, out_channels).cuda()
    
    print(summary(model,shape(rand_input)))
